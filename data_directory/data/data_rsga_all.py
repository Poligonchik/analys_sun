...
# --- Расширенные регулярные выражения с учетом вариантов написания (IGNORECASE) ---
RE_SDF_NUMBER = re.compile(r"SDF Number\s+(\d+)", re.IGNORECASE)
# Учитываем варианты: "Solar activity has been at" или "SOLAR ACTIVITY WAS" и т.п.
...
def parse_rsgi_file(filepath: Path) -> dict | None:
    ...
    try:
        ...
        return {
            "filename": filepath.name,  # имя файла для отладки
            "issued": issued,
            "product": product,
...
def main():
    ...
    # Обрабатываем файлы из папок для годов 1988-1995: путь "./ftp_data/<year>_RSGA"
    ...
    # Обрабатываем файлы из папок для годов 1996-2024: путь "../ftp_data/<year>/<year>_RSGA"
    ...
    # Сохраняем объединённые данные в JSON-файл
