...
def analyze_rsga_relationships():
    ...
    # зависимость между Solar Wind Speed Peak и Total IMF Max
    ...
    # Анализ распределения категориальных полей
...
def extract_section(text: str, section_name: str) -> str:
    """
    Извлекает содержимое секции из текста.
    """
...
def detailed_field_analysis():
    """
    Выводит общее число записей, число непустых значений
    Выводит таблицы распределения
    """
    output_directory = Path("../processed_results")
    json_path = output_directory / "rsga_combined_all.json"
    if not json_path.exists():
