...
# Функция извлечения класса вспышки из поля particulars
...
# Функция для расчёта признаков по типам вспышек за последние N часов (здесь N=24)
def add_flare_type_features(df, window_hours):
    ...
    # Отношение сильных вспышек (M и X) к общему числу (если total_count==0, задаём 0)
...
# новые признаки: last_flare_24h, count_A_24h, count_B_24h, count_C_24h, count_M_24h, count_X_24h, total_flare_count_24h, ratio_MX_24h
# таргет: target_24
...
# Для удобства можно удалить исходные поля, которые не участвуют в моделировании
df_model = df.copy()
df_model = df_model.drop(
    columns=['begin', 'end', 'particulars', 'loc_freq', 'region', 'flare_class', 'target_12', 'target_48',
             'target_24'])  # временно удалим таргет
# Затем добавим таргет отдельно (target_24)
df_model['target_24'] = df['target_24']
...
# Оставляем только выбранные признаки и таргет
df_final = df_model[features_to_use + ['target_24']].dropna().reset_index(drop=True)
...
#############################################
# Вывод метрик для моделей (предсказание на следующие 24 часа)
#############################################
